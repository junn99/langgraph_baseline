...
)  # MCP session management and startup parameters
...
from langchain_mcp_adapters.tools import (
    load_mcp_tools,
)  # Adapter to load MCP tools correctly
...
llm = ChatGoogleGenerativeAI(
    ...
    google_api_key=os.getenv(
        "GOOGLE_API_KEY"
    ),  # Google API key must be set in your environment or .env file
)
...
if len(sys.argv) < 2:
    print(
        "Usage: python client_langchain_google_genai_bind_tools.py <path_to_server_script>"
    )
    sys.exit(1)
